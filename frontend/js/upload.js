...
async function submitPaper() {
  ...
  status.textContent = "⏳ Extracting metadata from your PDF... please don't close or navigate away.";

  uploadInProgress = true;

  const emptyFormData = { title: "", department: "", year: "", members: "", abstract: "" };

  try {
    const preview = await apiIngestPreview(selectedFile, emptyFormData);
...
function showMetaModal(meta) {
  ...
  document.getElementById("metaAuthors").textContent  = meta.authors || "—";
  document.getElementById("metaYear").textContent     = meta.year || "—";
  document.getElementById("metaCollege").textContent  = meta.college || "—";
  document.getElementById("metaAbstract").textContent = meta.abstract || "—";
  document.getElementById("metaKeywords").textContent = meta.keywords || "—";
...
async function confirmMetaAndSubmit() {
  if (!currentPreviewId) { toast("Preview expired — please re-upload.", "error"); return; }
  ...
  status.style.display = "block";
  status.style.cssText = "display:block;padding:12px 14px;border-radius:7px;font-size:0.84rem;color:var(--muted);background:var(--bg);";
  status.textContent = "⏳ Uploading and indexing document into repository... please don't close or navigate away.";

  uploadInProgress = true;

  try {
    const result = await apiIngestConfirm(currentPreviewId, {});
    ...
    status.textContent   = `✅ "${result.metadata?.title || selectedFile.name}" submitted — ${result.chunks} chunks indexed. Pending librarian review.`;
